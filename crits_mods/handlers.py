...
def generate_fts(val):
    """
    Takes the value, removes surrounding quotes, and generates a pymongo $text
    query for full text search.
 
    :param val: The string to use for the full text search
    :type: val: str
    :returns: dict with key '$search' and value of the text search if successful, 'error' if failed.
    """

    try:
        return {'$search':val}
    except Exception, e:
        return {'error': 'Invalid Query: %s\n\n\t\%s' % (val, str(e))}
